...
if($_SESSION['pr']==0 AND $_SESSION['observe'] == 0)
{
    if(isset($_SESSION['gold']) AND $_SESSION['gold'] >= 50 )
{
//on enlève 50or (prix du magasin)
$_SESSION['gold'] = $_SESSION['gold']-50;
$insertgold = $bdd->prepare("UPDATE ressources SET gold = ? WHERE ID = ?");
$insertgold->execute(array($_SESSION['gold'],$_SESSION['ID']));
//on donne la potion
$_SESSION['pr'] = 1;
$insertWeapon = $bdd->prepare("UPDATE potion SET pr = ? WHERE DID = ?");
$insertWeapon->execute(array($_SESSION['pr'],$_SESSION['ShopPotionID']));
//énergie
